...
fn compound_assignment_covers_every_arithmetic_operator() {
    let out = run("var y = 5; y += 2; y -= 1; y *= 3; y /= 2; y %= 5; out(y);").unwrap();
    assert_eq!(out, ["4"]);
}

#[test]
fn increment_and_decrement_in_both_positions() {
    let out = run("var x = 1; x++; out(x); ++x; out(x); x--; out(x); --x; out(x);").unwrap();
...
fn increment_is_rejected_as_a_bare_binary_operand() {
    assert!(err("var x = 1; out(1 + x++);").contains("invalid increment/decrement target"));
    assert!(err("var x = 1; out(1 + x += 1);").contains("invalid assignment target"));
}
...
fn assignment_forms_embed_wherever_expression_is_re_entered() {
    assert_eq!(run("var x = 1; out(1 + (x++), x);").unwrap(), ["3 2"]);
    assert_eq!(run("var y = 1; out(1 + (++y), y);").unwrap(), ["3 2"]);
    assert_eq!(run("var z = 1; out([z += 5], z);").unwrap(), ["[6] 6"]);
    assert_eq!(run("var w = 1; out({k: (w += 1)});").unwrap(), ["{k: 2}"]);
    assert_eq!(run("var v = 1; out(v += 1);").unwrap(), ["2"]);
}
...
fn prefix_increment_takes_only_its_target_and_ends_the_expression() {
    assert!(err("var x = 1; out(++x + 1);").len() > 3);
    // parenthesising closes the assignment expression, after which an operator
    // continues normally
    assert_eq!(run("var x = 1; out((++x) + 1, x);").unwrap(), ["3 2"]);
    assert_eq!(run("var x = 1; out((x++) + 1, x);").unwrap(), ["3 2"]);
    assert_eq!(run("var x = 1; out(++x, x);").unwrap(), ["2 2"]);
}

#[test]
fn assignment_is_right_associative_and_targets_are_checked() {
    assert_eq!(
        run("var a = 0; var b = 0; a = b = 3; out(a, b);").unwrap(),
        ["3 3"]
    );
    assert!(err("1 = 2;").contains("invalid assignment target"));
    assert!(err("var x = 1; (x ? x : x) = 5;").contains("invalid assignment target"));
}

/// There's no declare-and-reassign form beyond `var`.
#[test]
...
fn map_keys_must_be_static() {
    assert!(err("var k = \"a\"; var m = {k + \"b\": 1};").len() > 3);
    assert_eq!(run(r#"var m = {a: 1}; out(m["a"]);"#).unwrap(), ["1"]);
}
...
fn deeply_nested_expressions_hit_the_parse_depth_guard() {
    let src = format!("var x = {}1{};", "(".repeat(400), ")".repeat(400));
