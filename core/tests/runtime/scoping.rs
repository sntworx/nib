...
fn blocks_create_and_drop_scopes() {
    assert_eq!(
        run("let x = 1; { let x = 2; out(x); } out(x);").unwrap(),
        ["2", "1"]
    );
...
#[test]
fn assignment_reaches_an_outer_scope_while_let_shadows() {
    assert_eq!(run("let x = 1; { x = 2; } out(x);").unwrap(), ["2"]);
    assert_eq!(run("let x = 1; { let x = 2; } out(x);").unwrap(), ["1"]);
}
...
fn for_init_is_scoped_to_the_loop() {
    assert_eq!(
        run("for (let i = 0; i < 2; i++) { out(i); }").unwrap(),
        ["0", "1"]
    );
    assert!(err("for (let i = 0; i < 1; i++) { } out(i);").len() > 3);
}
...
fn for_in_binding_is_scoped_to_the_loop() {
    ...
    assert_eq!(
        run("let x = 9; for x in [1, 2] { } out(x);").unwrap(),
        ["9"]
    );
}
...
fn catch_binding_is_scoped_to_the_catch_block() {
    ...
    assert_eq!(
        run(r#"let e = 1; try { throw "x"; } catch e { } out(e);"#).unwrap(),
        ["1"]
    );
}
...
fn if_and_while_bodies_have_their_own_scope() {
    assert!(err("if true { let a = 1; } out(a);").len() > 3);
    assert!(err("while true { let b = 1; break; } out(b);").len() > 3);
}

#[test]
fn redeclaring_a_global_overwrites_it() {
    assert_eq!(run("let x = 1; let x = 2; out(x);").unwrap(), ["2"]);
}
